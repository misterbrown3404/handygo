          ...
          return Center(
            child: Column(
              ...
              children: [
                ...
                Text(controller.error.value, style: const TextStyle(color: AdminColors.textSecondary)),
                const SizedBox(height: 16),
                ElevatedButton(onPressed: controller.fetchAnalytics, child: const Text('Retry')),
              ],
            ),
          );
        }
                ...
                builder: (context, constraints) {
                  if (constraints.maxWidth > 900) {
                    return Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Expanded(child: WeeklyJobsChart(data: controller.weeklyVolume)),
                      const SizedBox(width: 24),
                      Expanded(child: JobCategoryChart(data: controller.jobsByCategory)),
                    ]);
                  }
  ...
  Widget _kpiRow(AnalyticsController controller) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final items = [
          _kpiCard('Avg. Job Value', '₦${kpis['avg_job_value'] ?? '0'}', '+5.2%', Icons.trending_up_rounded),
          _kpiCard('Worker Retention', '${kpis['worker_retention'] ?? '0'}%', '+2.1%', Icons.people_rounded),
          _kpiCard('Active Subscriptions', '${kpis['active_subscriptions'] ?? '0'}', '+12%', Icons.card_membership_rounded),
          _kpiCard('Jobs This Month', '${kpis['jobs_this_month'] ?? '0'}', '-0.5%', Icons.task_alt_rounded),
        ];
        if (isWide) {
          ...
          Row(
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(color: AdminColors.success.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(6)),
      ...
      child: Column(
        ...
        children: [
          ...
          if (controller.topWorkers.isEmpty)
            const Center(child: Padding(padding: EdgeInsets.all(20), child: Text('No data available', style: TextStyle(color: AdminColors.textSecondary)))),
          ...controller.topWorkers.asMap().entries.map((entry) {
            ...
            return Container(
              ...
              decoration: BoxDecoration(border: Border(bottom: BorderSide(color: AdminColors.borderDark.withValues(alpha: 0.3)))),
              child: Row(
                children: [
                  Container(
                    width: 28,
                    ...
                    decoration: BoxDecoration(
                      color: rank == 1 ? AdminColors.warning.withValues(alpha: 0.2) : Colors.black.withValues(alpha: 0.04),
                      borderRadius: BorderRadius.circular(8),
                    ),
                    child: Center(
                        child: Text('#$rank',
                            style: TextStyle(
                                color: rank == 1 ? AdminColors.warning : AdminColors.textSecondary, fontSize: 12, fontWeight: FontWeight.bold))),
                  ),
                  const SizedBox(width: 16),
                  CircleAvatar(
                      radius: 16,
                      backgroundColor: AdminColors.primary.withValues(alpha: 0.15),
                      child: Text(w['name']?[0] ?? '?', style: const TextStyle(color: AdminColors.primary, fontSize: 12, fontWeight: FontWeight.bold))),
                  const SizedBox(width: 12),
                  Expanded(
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(w['name'] ?? 'Unknown', style: const TextStyle(color: AdminColors.textPrimary, fontWeight: FontWeight.w600, fontSize: 13)),
                      Text(w['specialty'] ?? 'N/A', style: const TextStyle(color: AdminColors.textSecondary, fontSize: 11)),
                    ]),
                  ),
                  _miniInfo(Icons.task_alt_rounded, '${w['jobs_count'] ?? 0} jobs'),
                  const SizedBox(width: 24),
                  _miniInfo(Icons.star_rounded, '${w['rating'] ?? 0.0}'),
                  const SizedBox(width: 24),
                  Text('₦${w['earnings'] ?? 0}', style: const TextStyle(color: AdminColors.primary, fontWeight: FontWeight.bold, fontSize: 13)),
                ],
              ),
            );
          }),
