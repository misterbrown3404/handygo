      ...
      child: Column(
        ...
        children: [
          const Row(
            ...
            children: [
              Text('Revenue Overview', style: TextStyle(color: AdminColors.textPrimary, fontSize: 18, fontWeight: FontWeight.bold)),
            ],
          ),
            ...
            child: LineChart(
              LineChartData(
                gridData: FlGridData(
                  ...
                  getDrawingHorizontalLine: (value) => FlLine(color: AdminColors.borderDark.withValues(alpha: 0.4), strokeWidth: 1),
                ),
                    ...
                    sideTitles: SideTitles(
                      ...
                      getTitlesWidget: (value, meta) {
                        ...
                        if (index < data.length) {
                          return Padding(padding: const EdgeInsets.only(top: 8), child: Text(data[index]['period'] ?? '', style: const TextStyle(color: AdminColors.textSecondary, fontSize: 11)));
                        }
                ...
                minX: 0, maxX: (data.length - 1).toDouble().clamp(0, double.infinity), minY: 0, maxY: maxY,
                lineBarsData: [
                  LineChartBarData(
                    ...
                    belowBarData: BarAreaData(
                      show: true,
                      gradient: LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [AdminColors.primary.withValues(alpha: 0.25), AdminColors.primary.withValues(alpha: 0.0)]),
                    ),
                    dotData: FlDotData(
                      show: true,
...
class JobCategoryChart extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final colors = [AdminColors.chart1, AdminColors.chart2, AdminColors.chart3, AdminColors.chart4, AdminColors.accent];
    ...
    return GlassCard(
      child: Column(
        ...
        children: [
          ...
          if (data.isEmpty)
            const SizedBox(height: 200, child: Center(child: Text('No data', style: TextStyle(color: AdminColors.textSecondary)))),
          if (data.isNotEmpty) ...[
            SizedBox(
              height: 200,
              child: PieChart(
            ...
            ...data.asMap().entries.map((e) => _legendItem(e.value['category'] ?? 'N/A', colors[e.key % colors.length])),
          ],
        ],
      ),
    );
...
class WeeklyJobsChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    double maxY = 20;
    if (data.isNotEmpty) {
      maxY = data.map((d) => (d['completed'] as num).toDouble()).reduce((a, b) => a > b ? a : b);
        ...
        children: [
          ...
          if (data.isNotEmpty)
            SizedBox(
              ...
              child: BarChart(
                BarChartData(
                  gridData: FlGridData(show: true, drawVerticalLine: false, horizontalInterval: maxY / 5, getDrawingHorizontalLine: (value) => FlLine(color: AdminColors.borderDark.withValues(alpha: 0.4), strokeWidth: 1)),
                  titlesData: FlTitlesData(
                    ...
                    bottomTitles: AxisTitles(sideTitles: SideTitles(showTitles: true, getTitlesWidget: (value, meta) {
                      final index = value.toInt();
                      if (index < data.length) {
                        return Padding(padding: const EdgeInsets.only(top: 8), child: Text(data[index]['day'] ?? '', style: const TextStyle(color: AdminColors.textSecondary, fontSize: 11)));
                      }
                      return const SizedBox.shrink();
                    })),
                    topTitles: const AxisTitles(sideTitles: SideTitles(showTitles: false)),
                  ...
                  barGroups: data.asMap().entries.map((e) {
                    return _barGroup(e.key, (e.value['completed'] as num).toDouble(), (e.value['pending'] as num).toDouble());
                  }).toList(),
                ),
              ),
            ),
