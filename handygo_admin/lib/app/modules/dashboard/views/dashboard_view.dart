          ...
          children: [
            GlassCard(
              ...
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  const Icon(Icons.calendar_today_rounded, color: AdminColors.textSecondary, size: 16),
                  const SizedBox(width: 8),
                  Text(now, style: const TextStyle(color: AdminColors.textSecondary, fontSize: 13)),
                ],
              ),
            ),
            GestureDetector(
      ...
      builder: (context, constraints) {
        if (constraints.maxWidth > 900) {
          return Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Expanded(child: WeeklyJobsChart(data: [])), // No weekly volume on dashboard yet
            const SizedBox(width: 24),
            Expanded(child: _recentActivity(c)),
          ]);
        }
        return Column(children: [const WeeklyJobsChart(data: []), const SizedBox(height: 24), _recentActivity(c)]);
      },
    );
  }
        ...
        child: Column(
          ...
          children: [
            ...
            Center(
              child: Column(
                children: [
                  Icon(Icons.inbox_rounded, color: AdminColors.textSecondary.withValues(alpha: 0.4), size: 48),
                  const SizedBox(height: 12),
                  const Text('No recent activity', style: TextStyle(color: AdminColors.textSecondary)),
                ],
              ),
            ),
          ],
      ...
      child: Column(
        ...
        children: [
          ...
          ...activities.map((a) {
            ...
            return _activityItem(a['title'] ?? '', a['sub'] ?? '', a['time'] ?? '', icon, color);
          }),
        ],
      ),
    );
