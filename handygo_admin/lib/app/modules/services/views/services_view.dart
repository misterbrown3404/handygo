...
class _ServicesAdminViewState extends State<ServicesAdminView> {
  ...
  Future<void> _deleteService(int id) async {
    ...
    if (confirm == true) {
      try {
        ...
        Get.snackbar('Success', 'Service deleted', backgroundColor: AdminColors.success, colorText: Colors.white);
        _fetchServices();
      } catch (e) {
        Get.snackbar('Error', 'Failed to delete service', backgroundColor: AdminColors.error, colorText: Colors.white);
      }
    }
  }
  ...
  void _showCreateEditDialog({Map<String, dynamic>? existing}) {
    ...
    final priceCtrl = TextEditingController(text: existing?['base_price']?.toString() ?? '');
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 12),
              _dialogField(descCtrl, 'Description', Icons.description_outlined, maxLines: 3),
              const SizedBox(height: 12),
              _dialogField(priceCtrl, 'Base Price (₦)', Icons.payments_outlined, keyboardType: TextInputType.number),
              const SizedBox(height: 12),
              _dialogField(iconCtrl, 'Icon URL (optional)', Icons.image_outlined),
            ],
          ),
        ),
        actions: [
          ...
          ElevatedButton(
            ...
            child: Text(existing != null ? 'Update' : 'Create', style: const TextStyle(color: Colors.white)),
          ),
        ],
      ),
    );
  ...
  Future<void> _saveService({
    ...
    if (name.isEmpty || category.isEmpty) {
      Get.snackbar('Error', 'Name and Category are required', backgroundColor: AdminColors.error, colorText: Colors.white);
      return;
    }
    ...
    try {
      if (id != null) {
        ...
        Get.snackbar('Success', 'Service updated', backgroundColor: AdminColors.success, colorText: Colors.white);
      } else {
        await _api.post(AdminApiConstants.adminServices, data: data);
        Get.snackbar('Success', 'Service created', backgroundColor: AdminColors.success, colorText: Colors.white);
      }
      Get.back();
      _fetchServices();
    } catch (e) {
      Get.snackbar('Error', 'Failed to save service', backgroundColor: AdminColors.error, colorText: Colors.white);
    }
  }
          ...
          children: [
            ...
            if (_isLoading)
              ...
              Center(
                child: Column(
                  children: [
                    Text(_error!, style: const TextStyle(color: AdminColors.error)),
                    const SizedBox(height: 12),
              ...
              GlassDataTable(
                ...
                rows: _services
                    .map((s) => DataRow(cells: [
                          ...
                          DataCell(Text(s['category'] ?? '', style: const TextStyle(color: AdminColors.textSecondary))),
                          DataCell(Text(
                            s['base_price'] != null ? '₦${s['base_price']}' : 'N/A',
                            style: const TextStyle(color: AdminColors.primary, fontWeight: FontWeight.bold),
                          )),
                          DataCell(StatusChip(label: (s['is_active'] == true || s['is_active'] == 1) ? 'Active' : 'Inactive')),
                          DataCell(Row(children: [
                            _actionIcon(Icons.edit_rounded, AdminColors.accent, 'Edit', () => _showCreateEditDialog(existing: s)),
                            const SizedBox(width: 8),
                            _actionIcon(Icons.delete_rounded, AdminColors.error, 'Delete', () => _deleteService(s['id'])),
                          ])),
                        ]))
                    .toList(),
              ),
    ...
    return Wrap(
      ...
      children: [
        const Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Services & Categories', style: TextStyle(color: AdminColors.textPrimary, fontSize: 28, fontWeight: FontWeight.bold)),
            SizedBox(height: 4),
            Text('Manage the service categories available on HandyGo', style: TextStyle(color: AdminColors.textSecondary, fontSize: 14)),
          ],
        ),
        ElevatedButton.icon(
